...
class ImageGradients(th.nn.Module):
  ...
  def __init__(self, c_in, use_sobel=True):
    ...
    if use_sobel:  # Sobel filters
      self.dx = th.nn.Conv2d(c_in, c_in, [3, 3], padding=1,
                             bias=False, groups=c_in)
      self.dy = th.nn.Conv2d(c_in, c_in, [3, 3], padding=1,
                             bias=False, groups=c_in)
      ...
      self.dx = th.nn.Conv2d(c_in, c_in, [1, 3], padding=(0, 1),
                             bias=False, groups=c_in)
      self.dy = th.nn.Conv2d(c_in, c_in, [3, 1], padding=(1, 0),
                             bias=False, groups=c_in)

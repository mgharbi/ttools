...
class TestTrainer(unittest.TestCase):
  ...
  def test_runs_validation(self):
    epochs = 3
    self.trainer.add_callback(self.callback)
    self.trainer.train(self.loader, num_epochs=epochs, val_dataloader=self.loader)
  ...
  def test_calls_interface(self):
    ...
    self.assertEqual(self.interface.fwd, 3*epochs*len(self.data))  # account for val fwd steps

...
class VisdomLoggingCallback(KeyedCallback):
  ...
  def __init__(self, keys=["loss"], val_keys=["loss"], port=8097, env="main"):
    super(VisdomLoggingCallback, self).__init__(keys=keys, val_keys=val_keys)
...
class CheckpointingCallback(Callback):
  ...
  def __init__(self, checkpointer, interval=600, max_files=5):
  ...
  def epoch_end(self):
    ...
    self.checkpointer.save("{}{}".format(CheckpointingCallback.EPOCH_PREFIX,
                                         self.epoch))

  def training_end(self):
    super(CheckpointingCallback, self).training_end()
    self.checkpointer.save("training_end")
  ...
  def __purge_old_files(self):
    ...
    p_chkpts = []
    for c in chkpts:
      if c.startswith(CheckpointingCallback.PERIODIC_PREFIX):
        p_chkpts.append(c)
